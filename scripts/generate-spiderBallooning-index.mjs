...
const spiderBallooningDir = path.join(ROOT, "spiderBallooning");
const outFile = path.join(spiderBallooningDir, "index.html");
...
async function main() {
  const st = await fs.stat(spiderBallooningDir).catch(() => null);
  if (!st || !st.isDirectory()) {
    console.error(`Missing directory: ${spiderBallooningDir}`);
    process.exit(1);
  }

  const entries = await fs.readdir(spiderBallooningDir, { withFileTypes: true });
...
<body>
  <h1>spiderBallooning/</h1>
  <p class="meta">Auto-generated on <code>${escHtml(now)}</code>. Do not edit manually.</p>

  ${
    htmlFiles.length
      ? `<ul>\n${items}\n    </ul>`
      : `<p class="empty">No HTML files found in <code>spiderBallooning/</code> (other than <code>index.html</code>).</p>`
  }
